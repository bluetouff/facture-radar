...
test("la bibliothèque publie exactement vingt-cinq questions à URL stable", () => {
  assert.equal(practicalQuestions.length, 25);
  assert.equal(new Set(practicalQuestions.map((question) => question.slug)).size, 25);
