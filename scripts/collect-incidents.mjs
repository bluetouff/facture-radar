...
async function fetchSource(source) {
  ...
  return new Promise((resolveBody, reject) => {
    ...
    const request = get(url, {
      ...
      headers: { "User-Agent": "PA-Check-public-status-review/1.0", "Accept": source.kind === "rss" ? "application/rss+xml, application/xml, text/xml" : "text/html", "Accept-Encoding": "identity" },
...
const results = await Promise.allSettled(watchSources.map(async source => {
  const body = await fetchSource(source);
  await writeFile(`${archive}/${source.id}.${source.kind === "rss" ? "xml" : "html"}`, body);
  const candidates = parseWatchSource(source, body);
...
console.log(JSON.stringify({ checkedAt, reviewFile: `${archive}/review.json`, changesToReview: review.changes.length, sources: review.sources.map(({ sourceId, status, candidateCount }) => ({ sourceId, status, candidateCount })) }));
